...
public class ArabicReshaper {
    ...
    /**
     * Searching for the letter and Get the right shape for the character
     * depends on the location specified
     * 
     * @param target
    ...
    private char getReshapedGlphy(char target, int location) {
        ...
        for (int n = 0; n < ARABIC_GLPHIES.length; n++) {
            ...
            if (ARABIC_GLPHIES[n][0] == target) {
                // Get the right shape for the character, depends on the
                // location
     ...
     * @return the integer number indicated the Number of forms the Character
     *         has, return 2 otherwise
     */
     ...
     * @param isEndOfWord
     *            Is those characters at the end of the Word, to get its right
     *            form
    ...
    public String reshapeIt(String unshapedWord) {
        ...
        reshapedWord.append( getReshapedGlphy( wordLetters[0], 2 ) );// 2 is the
                                                                     // Form
                                                                     // when the
                                                                     // Letter
                                                                     // is at
        ...
        for (int i = 1; i < wordLength - 1; i++) {
            int beforeLast = i - 1;
            // Check if the Letter Before Last has only 2 Forms, for the current
            // Letter to be as a start for a new Word!
            if (getGlphyType( wordLetters[beforeLast] ) == 2) { // checking if
                                                                // it's only has
                                                                // 2 shapes
                // If the letter has only 2 shapes, then it doesnt matter which
                // position it is, It'll be always the second form
                reshapedWord.append( getReshapedGlphy( wordLetters[i], 2 ) );
            } else {
                // Then it should be in the middle which should be placed in its
                // right form [3]
                reshapedWord.append( getReshapedGlphy( wordLetters[i], 3 ) );
            }
        }

        // check for the last letter Before last has 2 forms, that means that
        // the last Letter will be alone.
        if (wordLength >= 2) {
            if (getGlphyType( wordLetters[wordLength - 2] ) == 2) {
                ...
                // Put the right form of the character, 4 for the last letter in
                // the word
